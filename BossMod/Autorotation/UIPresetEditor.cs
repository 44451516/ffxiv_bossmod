...
public sealed class UIPresetEditor
{
    ...
    public void DrawModulesList()
    {
        ...
        if (ImGui.Button("添加##module", width))
            ImGui.OpenPopup("add_module");

        if (UIMisc.Button("移除##module", width.X, (_selectedModuleIndex < 0, "请选择要移除的模块"), (!ImGui.GetIO().KeyShift, "按住 Shift")))
        {
    ...
    private void DrawSettingsList()
    {
        ...
        {
            ImGui.TextUnformatted("添加或选择循环模块以配置其策略");
            return;
        }
        ...
        ImGui.Checkbox("显示隐藏轨道", ref _showHiddenTracks);
    ...
    private bool SuggestHealerAI(Preset.ModuleSettings ms)
    {
        if (!_currentModuleHasHealerAI && _autorotConfig.SuggestHealerAI && _misleadingHealerRotations.Contains(ms.Type))
        {
            ImGui.TableNextRow();
            ImGui.TableNextColumn();
            ImGui.TextUnformatted("自动治疗/自动复活");
            ImGui.TableNextColumn();
            ImGui.TextUnformatted("这不是标准循环的一部分。请改用治疗 AI 模块。");
            if (ImGui.Button("添加治疗 AI"))
            {
